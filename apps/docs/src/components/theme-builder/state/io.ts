...
export function parseThemeJson(text: string): ParseResult {
  ...
  const identityError = validateIdentity(obj);
  if (identityError) return { ok: false, error: identityError };

  const accentError = validateAccentSwatches(obj);
  if (accentError) return { ok: false, error: accentError };

  const sectionError = validateSections(obj);
...
function validateIdentity(obj: Record<string, unknown>): string | null {
  for (const required of ['id', 'name', 'description'] as const) {
    const v = obj[required];
...
function validateAccentSwatches(obj: Record<string, unknown>): string | null {
  if (!Array.isArray(obj.accentSwatches)) {
    return 'Missing or invalid field: accentSwatches.';
  }
  for (const swatch of obj.accentSwatches) {
    if (typeof swatch !== 'string') {
      return 'accentSwatches must be an array of strings.';
    }
  }
  return null;
}
