...
(scalars → semantic roles → component tokens), its `applyTheme` runtime,
its primitives (`cn`, `cssVars`, `with-slot`, `useIsMobile`), and its
component catalogue.
...
- **Diagnose theming surprises**: when a component "doesn't pick up" a
  ...
  default → semantic role → scalar — and identify which layer is
  pinning the value.
...
- Inventing token names. If the user asks for a tone you don't see, ask
  ...
  or override a component token (per-component tweak) — the answer
  shapes the PR.
